...
function division(number, dividedBy) {
    ...
    return "error you cannot divide By 0";
        ...
        return 0;
    }else if((number < 0 && dividedBy < 0) || (number>0 &&dividedBy>0)){
        
        return 1 + division(Math.abs(number) - Math.abs(dividedBy), Math.abs(dividedBy));

        
    }else{
        return (1 + division(Math.abs(number) - Math.abs(dividedBy), Math.abs(dividedBy)) )*-1;
